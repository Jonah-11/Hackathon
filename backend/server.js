...
app.post('/jobListings', async (req, res) => {
    ...
    if (!contact_email) return res.status(400).json({ message: "Contact email is required." });
    if (!contact_phone) return res.status(400).json({ message: "Contact phone is required." });
    if (!deadline) return res.status(400).json({ message: "Deadline is required." }); // Validate deadline
